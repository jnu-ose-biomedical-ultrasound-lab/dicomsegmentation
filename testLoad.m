...
dicom=array;
targets=find(dicom>12 & dicom<75);
targetsBone=find(dicom>299);
d2=zeros(size(dicom));
d2(targets)=dicom(targets);
dBones=zeros(size(dicom));
dBones(targetsBone)=dicom(targetsBone);
name1='bloodDicom';
name2='boneDicom';
ext='.dcm';
for nu=1:tLength;
...
X1=squeeze(d2(:,:,nu));    
fileNo=num2str(nu);
fileEnd=strcat(fileNo,ext);
bloodName = strcat(name1,fileEnd);
dicomwrite(X1,bloodName)

X2=squeeze(dBones(:,:,nu));    


boneName = strcat(name2,fileEnd);
dicomwrite(X1,boneName)
